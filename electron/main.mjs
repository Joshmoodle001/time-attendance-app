...
async function pollRemoteReportJobs() {
  ...
  try {
    ...
    // Only include reportPayload if it has the required criteria structure
    if (result?.reportPayload?.criteria?.templateKey) {
      completePayload.reportPayload = result.reportPayload;
    } else if (result?.reportPayload) {
      logBridge(`Remote job ${job.jobId} result payload missing criteria — sending as-is`);
      completePayload.reportPayload = result.reportPayload;
    }

    await postRemoteBridge("/api/report-jobs-complete", completePayload);
